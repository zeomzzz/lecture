...
balls.append({
    "pos_x" : 50, # 공의 x좌표
    "pos_y" : 50, # 공의 y좌표
    "img_idx" : 0, # 공의 이미지 인덱스 (어떤 공을 사용할지)
    "to_x" : 3, # x축 이동방향, -3이면 왼쪽, 3이면 오른쪽
    "to_y": -6, # y축 이동방향,
    "init_spd_y": ball_speed_y[0]}) # y 최초 속도 

# 사라질 무기, 공 정보 저장 변수
weapon_to_remove = -1 # -1 이 삭제할 공/무기 판별 기준이 됨
ball_to_remove = -1
...
while running :
    ...
    # 공 위치 정의
    for ball_idx, ball_val in enumerate(balls): # 인덱스와 이에 해당하는 value 가져옴
    ...
    if weapon_to_remove > -1 :
        ...
        weapon_to_remove = -1 # 충돌한 idx의 무기 삭제하고 weapon_to_remove 값은 -1 로 초기화
